...
def data_cleansing(count_file, rain_file, temp_file, solar_file):
    ...
    rainfall_df.drop(["Product code","Bureau of Meteorology station number", \
        "Period over which rainfall was measured (days)","Quality", "Year", "Month", "Day"],axis = 1, inplace = True)
    ...
    temperature_df.drop(["Product code","Bureau of Meteorology station number",  "Year", "Month", \
        "Day", "Days of accumulation of maximum temperature", "Quality"],axis = 1, inplace = True)
    ...
    | df["Maximum temperature (Degree C)"].isnull() | df["Daily global solar exposure (MJ/m*m)"].isnull())
