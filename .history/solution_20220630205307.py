...
def timeSeriesSensor(df, year1, year2, month):
    may_2021 = pd.DataFrame(df[(df.Year == year1) & (df.Month == month)].groupby([df.Sensor_ID, df.Date_Time.dt.strftime('%m-%d')]).agg({"Hourly_Counts":"sum"}))
    may_2022 = pd.DataFrame(df[(df.Year == year2) & (df.Month == month)].groupby([df.Sensor_ID, df.Date_Time.dt.strftime('%m-%d')]).agg({"Hourly_Counts":"sum"}))
    compare_merged = pd.merge(left=may_2021, right=may_2022, left_on=['Sensor_ID','Date_Time'], right_on=['Sensor_ID','Date_Time'])
    e_distance = defaultdict(float)
    n = compare_merged['Sensor_ID'].max()
    # compute euclidean distance
    for sensor in range(1,n):
        count_2021 = []
        count_2022 = []
    ...
    print("Sensor with the most change is sensor_id = " + str(max_sensor) + ", and the greatest change is " + str(round(max_change)) + ".")
