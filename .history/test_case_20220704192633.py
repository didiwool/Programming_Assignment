...
class TestSummaryHourlyCount(unittest.TestCase):
    ...
    def test_random_hours(self):
        dataframe = data_cleansing(COUNT_TEST, RAIN_TEST, TEMP_TEST, SOLAR_TEST)
        summary = summary_hourly_count(dataframe, 12)
        result = dataframe['Hourly_Counts'].sum()
        print(result)
        print(summary.iloc[0]['max'].astype(float))
        self.assertEqual(True, summary.equals(result))
