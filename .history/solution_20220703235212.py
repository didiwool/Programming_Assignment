...
def data_cleansing(count_file, rain_file, temp_file, solar_file):
    ...
    pedestrain_df["date_key"] = pedestrain_df["Year"].astype(str) \
        + pedestrain_df["Month"].map(month_dict).astype(str) + pedestrain_df["Mdate"].astype(str)
    ...
    rainfall_df.drop(["Product code",
        "Bureau of Meteorology station number", "Period over which rainfall was measured (days)", \
    ...
    xcut = (df["Rainfall amount (millimetres)"].isnull() \
        | df["Maximum temperature (Degree C)"].isnull() \
        | df["Daily global solar exposure (MJ/m*m)"].isnull())
...
def pedestrianScatter(df, year1, year2, x_axis):
    daily_overall_2021 = pd.DataFrame(df[df.Year == year1] \
        .groupby(df.Date_Time.dt.strftime('%y-%m-%d')).agg({'Hourly_Counts':'sum',x_axis:'mean'}))
    daily_overall_2022 = pd.DataFrame(df[df.Year == year2] \
        .groupby(df.Date_Time.dt.strftime('%y-%m-%d')).agg({'Hourly_Counts':'sum',x_axis:'mean'}))
...
def pedestrianHist(df, year1, year2):
    df1 = pd.DataFrame(df[df.Year == year1]. \
        groupby([df.Date_Time.dt.strftime('%y-%m-%d'), df.Day]) \
    ...
    daily_count(df1, title1, "Day of week", "mean daily overall pesdestrain count", file1)

    df2 = pd.DataFrame(df[df.Year == year2] \
        .groupby([df.Date_Time.dt.strftime('%y-%m-%d'), df.Day]) \
        .agg({'Hourly_Counts':'sum'})).groupby('Day').mean()
    df2 = daily_count(df2)
    ...
    daily_count(df2, title2, "Day of week", "mean daily overall pesdestrain count", file2)


# general function of question 6
def sensorHist(df, year, start_no, end_no):
    new_df= pd.DataFrame(pd.DataFrame(df[(df.Year == year) \
        & (df.Sensor_ID.isin(range(start_no, end_no+1)))] \
        .groupby([df.Sensor_ID, df.Date_Time.dt.strftime('%y-%m-%d')]) \
    ...
    bar_with_title(new_df, title, "sensor ID", "mean daily overall pesdestrain count", file)


# the general function of question 7
def pedestrianHistRain(df, year1, year2):
    df1 = pd.DataFrame(df[(df.Year == year1) \
        & (df["Rainfall amount (millimetres)"]>0)] \
        .groupby([df.Date_Time.dt.strftime('%y-%m-%d'), df.Day]) \
        .agg({'Hourly_Counts':'sum'})).groupby('Day').mean()
...
def pedestrianHistRainTemp(df, year1, year2, max_temp):
    df1 = pd.DataFrame(df[(df.Year == year1) \
        & (df["Rainfall amount (millimetres)"]>0) \
        & (df["Maximum temperature (Degree C)"] < max_temp)] \
        .groupby([df.Date_Time.dt.strftime('%y-%m-%d'), df.Day]).agg({'Hourly_Counts':'sum'}))
    ...
    df2 = pd.DataFrame(df[(df.Year == year2) & \
        (df["Rainfall amount (millimetres)"]>0) & (df["Maximum temperature (Degree C)"] < max_temp)] \
        .groupby([df.Date_Time.dt.strftime('%y-%m-%d'), df.Day]).agg({'Hourly_Counts':'sum'}))
...
def timeSeriesSensor(df, year1, year2, month):
    df1 = pd.DataFrame(df[(df.Year == year1) & (df.Month == month)] \
        .groupby([df.Sensor_ID, df.Date_Time.dt.strftime('%m-%d')]).agg({"Hourly_Counts":"sum"}))
    df2 = pd.DataFrame(df[(df.Year == year2) & (df.Month == month)] \
        .groupby([df.Sensor_ID, df.Date_Time.dt.strftime('%m-%d')]).agg({"Hourly_Counts":"sum"}))
    ...
    print("Sensor with the most change is sensor_id = " + str(max_sensor) + \
        ", and the greatest change is " + str(max_change) + ". The name of the sensor is: " + max_sensor_name)

    # plot and save scatter plot of max sensor
    count_2021_max_sensor = pd.DataFrame(df[(df.Sensor_ID == max_sensor) \
        & (df.Month == month) & (df.Year == year1)] \
        .groupby(df.Date_Time.dt.strftime('%m-%d')).sum()['Hourly_Counts'])
    count_2021_max_sensor.reset_index()
    count_2022_max_sensor = pd.DataFrame(df[(df.Sensor_ID == max_sensor) \
        & (df.Month == month) & (df.Year == year2)] \
        .groupby(df.Date_Time.dt.strftime('%m-%d')).sum()['Hourly_Counts'])
    count_2021_max_sensor.reset_index()

    title1 = "Bar plot for sensor with maximum change in " + month + " " + str(year1)
    file1 = str(year1) + "_max_change_sensor.png"
    bar_with_title(count_2021_max_sensor, title1, "Date", "Total daily overall pesdestrain count", file1)

    title2 = "Bar plot for sensor with maximum change in " + month + " " + str(year2)
    file2 = str(year2) + "_max_change_sensor.png"
    bar_with_title(count_2022_max_sensor, title2, "Date", "Total daily overall pesdestrain count", file2)
...
def modelForCount(df, sensor_id, start_time, end_time):
    ...
    train_data = np.array(df[(df.Sensor_ID == sensor_id) & \
        (df.Time >= start_time) & (df.Time <= end_time-1) & (df.Year ==2022) \
        & (df.Month.isin(["January", "February", "March", "April"])) \
        & (df.Date_Time.dt.strftime('%m-%d') != '01-01')].sort_values(by = ['Date_Time'])['Hourly_Counts'])
    (rain_prev, solar_prev, temp_prev, sensor3_past1, nearby_past1, sensor3_pastday) \
        = data_for_count(df, nearby, sensor_id, start_time, end_time)
    ...
    for day in WEEK:
        (rain_prev, solar_prev, temp_prev, sensor3_past1, nearby_past1, sensor3_pastday) \
            = test_data_for_count(df, nearby, sensor_id, start_time, end_time, day)
        x_test = np.concatenate((rain_prev.reshape(-1,1), solar_prev.reshape(-1,1), \
            temp_prev.reshape(-1,1), sensor3_past1.reshape(-1,1), nearby_past1.reshape(-1,1), \
            sensor3_pastday.reshape(-1,1)), axis = 1)
        y_test = np.array(df[(df.Day == day) & (df.Sensor_ID == 3) & (df.Time == 12) \
            & (df.Year ==2022) & (df.Month == 'May' ) & (df.Date_Time.dt.strftime('%m-%d') != '05-01')] \
...
def unusualDay(df, sensor_id):
    ...
    sensor3_pastday = np.array(df[(df.Sensor_ID == sensor_id) & (df.Year ==2022) \
        & (df.Month.isin(["January", "February", "March", "April", "May"])) \
        & (df.Date_Time.dt.strftime('%m-%d') != '05-31')].sort_values(by = ['Date_Time'])['Hourly_Counts'])

    x = np.concatenate((rain_prev.reshape(-1,1), solar_prev.reshape(-1,1), \
        temp_prev.reshape(-1,1), sensor2_pastday.reshape(-1,1), sensor3_pastday.reshape(-1,1)), axis = 1)
    y = train_data

    model = LinearRegression().fit(x, y)
    new_df = df[(df.Sensor_ID == 3) & (df.Time >= 0) & (df.Time <= 23) & (df.Year ==2022) \
        & (df.Month.isin(["January", "February", "March", "April", "May"])) \
    ...
    new_df = new_df.groupby(['Month', 'Mdate', 'Sensor_ID', 'Day', 'Year', \
        ...
        'Daily global solar exposure (MJ/m*m)']).agg({'distance':'sum'}).reset_index()
    ...
    for i in range(3):
        ...
        train_data = np.array(df[(df.Sensor_ID == sensor_id) & (df.Year ==2022) \
            & (df.Month == month) & (df.Mdate == mdate)].sort_values(by = ['Date_Time'])['Hourly_Counts'])
        rain_prev = np.array(df[(df.Sensor_ID == sensor_id) & (df.Year ==2022) \
        ...
        temp_prev = np.array(df[(df.Sensor_ID == sensor_id) & (df.Year ==2022) \
            ...
            .sort_values(by = ['Date_Time'])['Maximum temperature (Degree C)'])
        sensor2_pastday = np.array(df[(df.Sensor_ID == nearby) & (df.Year ==2022) \
            & (df.Month == lastmonth) & (df.Mdate == lastday)].sort_values(by = ['Date_Time'])['Hourly_Counts'])
        sensor3_pastday = np.array(df[(df.Sensor_ID == sensor_id) & (df.Year ==2022) \
            & (df.Month == lastmonth) & (df.Mdate == lastday)].sort_values(by = ['Date_Time'])['Hourly_Counts'])
...
def dailyDifference(df, sensor1, sensor2):
    sensor3 = df[(df.Year == 2022) & (df.Month == 'May') \
        & (df.Sensor_ID == sensor1)][["Time", "Date_Time", "Hourly_Counts"]]
    sensor3["Date_Time"] = sensor3.Date_Time.dt.strftime('%m-%d')

    sensor9 = df[(df.Year == 2022) & (df.Month == 'May') \
        & (df.Sensor_ID == sensor2)][["Time", "Date_Time", "Hourly_Counts"]]
...
def sensorCorrelation(df, sensor1, sensor2):
    sensor3 = df[(df.Year == 2022) & (df.Month == 'May') & (df.Sensor_ID == sensor1) \
        & (df.Time >= 9) & (df.Time <= 17) & (df.Day.isin(WEEKDAY))][["Time", "Date_Time", "Hourly_Counts"]]
    sensor3["Date_Time"] = sensor3.Date_Time.dt.strftime('%m-%d')

    sensor9 = df[(df.Year == 2022) & (df.Month == 'May') & (df.Sensor_ID == sensor2) \
        & (df.Time >= 9) & (df.Time <= 17) & (df.Day.isin(WEEKDAY))][["Time", "Date_Time", "Hourly_Counts"]]
    sensor9["Date_Time"] = sensor9.Date_Time.dt.strftime('%m-%d')
...
def joinCovidTravel(df, file1, file2):
    ...
    covid_pedestrain_df = covid_pedestrain_df \
        .groupby(covid_pedestrain_df.Date_Time, as_index=False).agg({'Hourly_Counts':'sum','VIC':'mean'})
...
def investCovidTravel(df, file1, file2):
    ...
    time_series_for_two(monthly_overall, "Arrival", "Hourly_Counts", \
        ...
        "Time serie data for monthly internation arrival and average pedestrain count")
    ...
    time_series_for_two(monthly_overall[pd.to_datetime(monthly_overall.Date_Time) >= '2021-09-01'], \
        "VIC", "Hourly_Counts", 'Average daily covid-19 cases', 'Average daily pedestrain count', \
        "Time serie data for monthly covid-19 cases and average pedestrain count before September 2021", '2')
...
def investLockdown(df):
    ...
    df_time_series.plot(x = "Date_Time", y = "Hourly_Counts", \
        title = "Covid-19 cases versus pedestrain (Red = Victoria under lockdown)", figsize=(12, 5))
