...
def data_cleansing(count_file, rain_file, temp_file, solar_file):
    ...
    df = df[xcut is False].reset_index(drop = True)
...
def pedestrian_scatter(df, year1, year2, x_axis):
    daily_overall_2021 = pd.DataFrame(df[df.Year == year1] \
        .groupby(df.Date_Time.dt.strftime('%y-%m-%d')) \
...
def pedestrian_hist(df, year1, year2):
...
def sensor_hist(df, year, start_no, end_no):
...
def pedestrian_hist_rain(df, year1, year2):
    df1 = pd.DataFrame(df[(df.Year == year1) \
...
def pedestrian_hist_rain_temp(df, year1, year2, max_temp):
    df1 = pd.DataFrame(df[(df.Year == year1) \
        & (df["Rainfall amount (millimetres)"]>0) \
...
def time_series_sensor(df, year1, year2, month):
...
def model_for_count(df, sensor_id, start_time, end_time):
        # q10
    # take variables:
    ...
    for day in WEEK:
        ...
        y_test = np.array(df[(df.Day == day) & (df.Sensor_ID == 3) & (df.Time == 12) \
            & (df.Year ==2022) & (df.Month == 'May' ) \
            & (df.Date_Time.dt.strftime('%m-%d') != '05-01')] \
            .sort_values(by = ['Date_Time'])['Hourly_Counts'])
...
def daily_difference(df, sensor1, sensor2):
...
def sensor_correlation(df, sensor1, sensor2):
    sensor3 = df[(df.Year == 2022) & (df.Month == 'May') \
        & (df.Sensor_ID == sensor1) & (df.Time >= 9) & (df.Time <= 17) \
        & (df.Day.isin(WEEKDAY))][["Time", "Date_Time", "Hourly_Counts"]]
    sensor3["Date_Time"] = sensor3.Date_Time.dt.strftime('%m-%d')

    sensor9 = df[(df.Year == 2022) & (df.Month == 'May') \
        & (df.Sensor_ID == sensor2) & (df.Time >= 9) & (df.Time <= 17) \
        & (df.Day.isin(WEEKDAY))][["Time", "Date_Time", "Hourly_Counts"]]
    sensor9["Date_Time"] = sensor9.Date_Time.dt.strftime('%m-%d')
...
def invest_covid_travel(df, file1, file2):
    """
...
def invest_lockdown(df):
    ...
    df_array = df[df['Date_Time'] < '2021-10-21'] \
