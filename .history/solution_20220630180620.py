...
def pedestrianHist(df, year1, year2):
    df1 = pd.DataFrame(df[df.Year == year1].groupby([df.Date_Time.dt.strftime('%y-%m-%d'), df.Day]).agg({'Hourly_Counts':'sum'}))
    df1 = dailyCount(df1)
    ...
    df2 = pd.DataFrame(df[df.Year == year2].groupby([df.Date_Time.dt.strftime('%y-%m-%d'), df.Day]).agg({'Hourly_Counts':'sum'}))
    df2 = dailyCount(df2)
...
def pedestrianHistRain(df, year1, year2):
    df1 = pd.DataFrame(df[(df.Year == year1) & (df["Rainfall amount (millimetres)"]>0)].groupby([df.Date_Time.dt.strftime('%y-%m-%d'), df.Day]).agg({'Hourly_Counts':'sum'}))
    ...
    file2 =  str(year1)+"_busy_daily_rain.png"
    barWithTitle(df2, title2, "Day of week", "mean daily overall pesdestrain count", file2)
