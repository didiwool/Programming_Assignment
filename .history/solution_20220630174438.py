...
def sensorHist(df, year1, year2, range):
    mean_count_2022 = pd.DataFrame(pd.DataFrame(df[(df.Year == 2022) & (df.Sensor_ID <= 20)].groupby([df.Sensor_ID, df.Date_Time.dt.strftime('%y-%m-%d')]).agg({'Hourly_Counts':'sum'}))).groupby('Sensor_ID').mean()
    mean_count_2022.plot(kind = "bar")
    plt.title("Mean daily overall pedestrain count for sensor 1-20 in 2022")
    plt.xlabel("sensor ID")
    plt.ylabel("mean daily overall pesdestrain count")
    plt.savefig('2022_busy_sensor.png')
