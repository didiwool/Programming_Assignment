...
def pedestrian_hist_rain(dataframe, year1, year2):
    ...
    df2 = pd.DataFrame(df[(df.Year == year2) \
        & (dataframe["Rainfall amount (millimetres)"]>0)] \
        .groupby([df.Date_Time.dt.strftime('%y-%m-%d'), dataframe.Day]) \
        .agg({'Hourly_Counts':'sum'})).groupby('Day').mean()
...
def pedestrian_hist_rain_temp(dataframe, year1, year2, max_temp):
    df1 = pd.DataFrame(dataframe[(dataframe.Year == year1) \
        ...
        .groupby([dataframe.Date_Time.dt.strftime('%y-%m-%d'), dataframe.Day]).agg({'Hourly_Counts':'sum'}))
...
# q15
def joinCovidTravel(dataframe, file1, file2):
    """
...
def invest_covid_travel(dataframe, file1, file2):
    ...
    """
    # join dataframe
    monthly_overall = joinCovidTravel(dataframe, file1, file2)
