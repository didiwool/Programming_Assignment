...
def modelForCount(df, sensor_id, start_time, end_time):
    ...
    # compute test data
    for day in WEEK:
        (rain_prev, solar_prev, temp_prev, sensor3_past1, nearby_past1, sensor3_pastday) = test_data_for_count(df, nearby, sensor_id, start_time, end_time, day)
        ...
        y_test = np.array(df[(df.Day == day) & (df.Sensor_ID == 3) & (df.Time == 12) & (df.Year ==2022) &(df.Month == 'May' ) & (df.Date_Time.dt.strftime('%m-%d') != '05-01')].sort_values(by = ['Date_Time'])['Hourly_Counts'])
